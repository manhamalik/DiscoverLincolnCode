...
const CreateForm = ({ tagsData }) => {
	...
	tagsData.usersList.forEach((element) =>
		usersMap.set(element.username, element)
	);
	tagDataObj.forEach((element) =>
	...
	function handleSubmit(event) {
		...
		try {
			...
			Array.from(selectedTags).forEach((element) =>
				selectedTagsId.push({ id: tagsList.get(element).id })
			);
			...
			if (
				selectedlatlng.latLng.lat === undefined ||
			...
			const formExport = {
				...
				Dates: {
					StartDate:
						data.get("start-date") !== "" ? data.get("start-date") : null,
					EndDate: data.get("end-date") !== "" ? data.get("end-date") : null,
					StartTime:
						data.get("time-start") !== ""
							? `${data.get("time-start")}:00`
							: null,
					EndTime:
						data.get("time-end") !== "" ? `${data.get("time-end")}:00` : null,
				},
				Location: {
			...
			console.log("trying to submit", formExport);
			...
			<div className="create-page-container">
				<div className="catagory-button-container">
					<div className="button-container">
						<Button
							type={!catagory[0] ? "inverted" : "primary"}
							onClick={() => handleCatagoryChange(0)}
						>
							Events
						</Button>
					</div>
					<div className="button-container">
						<Button
							type={!catagory[1] ? "inverted" : "primary"}
							onClick={() => handleCatagoryChange(1)}
						>
							Attractions
						</Button>
					</div>
					<div className="button-container">
						<Button
							type={!catagory[2] ? "inverted" : "primary"}
							onClick={() => handleCatagoryChange(2)}
						>
							Businesses
						</Button>
					</div>
...
async function sendForm(formData, images, categoryName) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (!res.ok) {
				throw new Error(
					`Form was unable to be submitted: ${response.error.message}`
				);
			} else {
				toast.success(
					`Successfully uploaded your ${
						categoryName === "businesses"
							? "business"
							: categoryName.slice(0, categoryName.length - 1)
			...
			toast.error(
				`Your ${categoryName} was unable to be submitted: ${error.message}`
			);
			reject(error);
		}
	});
...
export async function getServerSideProps() {
	try {
		const [response, users] = await Promise.all([
			...
			await fetch(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/users`),
		]);
